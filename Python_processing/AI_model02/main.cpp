...
Solution build_initial_solution_stage1(
    ...
    for (auto &kv : cust_by_depot) {
        ...
        sort(unassigned.begin(), unassigned.end(), [&](int a, int b) {
            double da = haversine_km(dep.lat, dep.lon, inter_customers[a].lat, inter_customers[a].lon);
            double db = haversine_km(dep.lat, dep.lon, inter_customers[b].lat, inter_customers[b].lon);
            return da < db;
        });

        while (!unassigned.empty()) {
            int start_idx = unassigned.front();
            unassigned.erase(unassigned.begin());

            string vid;
            double best_cap = -1.0;
            for (auto &vid_candidate : veh_ids) {
                auto itv = veh_by_id.find(vid_candidate);
                if (itv == veh_by_id.end()) continue;
                if (itv->second.cap_weight > best_cap) {
                    best_cap = itv->second.cap_weight;
                    vid = vid_candidate;
                }
            }
            if (vid.empty()) {
                cerr << "[MAIN] WARNING: cannot choose vehicle for depot " << depot_id << "\n";
                break;
            }

            const auto &vrow = veh_by_id[vid];
            double capw = vrow.cap_weight;
            double capv = vrow.cap_volume;
            double max_dist = vrow.max_distance;
            double max_time_min = vrow.max_hours * 60.0;

            vector<int> route_custs;
            route_custs.push_back(start_idx);
            double cur_w = inter_customers[start_idx].weight;
            double cur_v = inter_customers[start_idx].volume;
            double cur_svc = inter_customers[start_idx].service_time;
            double cur_dist = compute_route_distance_for_trip(route_custs, inter_customers, dep);

            bool extended = true;
            while (extended && !unassigned.empty()) {
                extended = false;
                int best_cand = -1;
                double best_new_dist = 0.0;

                for (int idx = 0; idx < (int)unassigned.size(); ++idx) {
                    int cand = unassigned[idx];
                    double new_w = cur_w + inter_customers[cand].weight;
                    double new_v = cur_v + inter_customers[cand].volume;

                    if (capw > 0 && new_w > capw * capacity_util) continue;
                    if (capv > 0 && new_v > capv * capacity_util) continue;

                    vector<int> tmp = route_custs;
                    tmp.push_back(cand);
                    double dist_tmp = compute_route_distance_for_trip(tmp, inter_customers, dep);
                    double travel_min_tmp = (AVERAGE_SPEED_KMPH > 0.0)
                                            ? (dist_tmp / AVERAGE_SPEED_KMPH * 60.0)
                                            : 0.0;
                    double svc_tmp = cur_svc + inter_customers[cand].service_time;
                    double time_tmp = travel_min_tmp + svc_tmp;

                    if (max_dist > 0 && dist_tmp > max_dist) continue;
                    if (max_time_min > 0 && time_tmp > max_time_min) continue;

                    double delta = dist_tmp - cur_dist;
                    if (!extended || delta < (best_new_dist - cur_dist)) {
                        extended = true;
                        best_cand = idx;
                        best_new_dist = dist_tmp;
                    }
                }

                if (extended && best_cand != -1) {
                    int cand = unassigned[best_cand];
                    route_custs.push_back(cand);
                    cur_w  += inter_customers[cand].weight;
                    cur_v  += inter_customers[cand].volume;
                    cur_svc += inter_customers[cand].service_time;
                    cur_dist = best_new_dist;
                    unassigned.erase(unassigned.begin() + best_cand);
                }
            }
            ...
            r.depot_id = depot_id;
            r.vehicle_id = vid;
            r.cust_idx = route_custs;
            routes.push_back(r);
            total_dist_est += cur_dist;
        }
    }
    ...
    cout << "[MAIN] Initial solution (intermediate): " << routes.size() << " routes\n";
    cout << "[MAIN] Initial total distance (approx, km): " << total_dist_est << "\n";
    return sol;
}
    ...
    Solution run(
        ...
        for (int it = 0; it < max_iters; ++it) {
            ...
            for (int trial = 0; trial < neighborhood_size; ++trial) {
                ...
                for (int r_idx = 0; r_idx < (int)current.routes.size(); ++r_idx) {
                    if (!current.routes[r_idx].cust_idx.empty())
                        non_empty_routes.push_back(r_idx);
                }
                if (non_empty_routes.empty()) break;

                int src_route_idx = non_empty_routes[rng() % non_empty_routes.size()];
                const auto &src_route = current.routes[src_route_idx];
                int src_pos = rng() % (int)src_route.cust_idx.size();
                int moved_cust = src_route.cust_idx[src_pos];

                vector<int> dest_routes;
                for (int r_idx = 0; r_idx < (int)current.routes.size(); ++r_idx) {
                    if (current.routes[r_idx].depot_id == src_route.depot_id) {
                        dest_routes.push_back(r_idx);
                    }
                }
                if (dest_routes.empty()) continue;

                int dst_route_idx = dest_routes[rng() % dest_routes.size()];

                Solution neigh = current;
                auto &r_src = neigh.routes[src_route_idx];
                auto &r_dst = neigh.routes[dst_route_idx];

                int cid_idx = r_src.cust_idx[src_pos];
                r_src.cust_idx.erase(r_src.cust_idx.begin() + src_pos);

                int dst_pos = rng() % ((int)r_dst.cust_idx.size() + 1);
                r_dst.cust_idx.insert(r_dst.cust_idx.begin() + dst_pos, cid_idx);

                if (!route_feasible(r_src) || !route_feasible(r_dst)) {
                    continue;
                }
                ...
                bool tabu = is_tabu(tabu_list, moved_cust, it);
                ...
                if ((!tabu || aspir) && neigh_cost < best_neigh_cost) {
                    ...
                    best_neigh = neigh;
                    best_moved_customer = moved_cust;
                    found = true;
                }
            }
