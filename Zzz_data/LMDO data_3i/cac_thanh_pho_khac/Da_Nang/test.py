...
def apply_move(sol: Solution, move: Move, inst: Instance) -> Solution:
    """Apply a relocate or swap move (already validated)."""
    new_sol = sol.copy()
...
# ============================================================
# GENERATE NEIGHBORS – NO CROSS CLUSTER / NO CROSS DEPOT
# ============================================================
...
def generate_neighbors(sol: Solution, inst: Instance,
    ...
    # map: customer → assigned cluster (depot)
    cust_cluster = inst.customer_cluster

    # list all customers with positions
    customer_positions = []
    for vid, route in sol.routes.items():
    ...
    for _ in range(max_neighbors // 2):
        ...
        cid_cluster = cust_cluster[cid]     # cluster of this customer
        depot_from = veh_depot[vid_from]

        # candidate vehicles must:
        # 1) start from same depot
        # 2) same assigned cluster
        veh_candidates = [
            vid for vid in veh_ids
            if veh_depot[vid] == depot_from
        ]

        # choose vid_to
        vid_to = rng.choice(veh_candidates)
        r_to = sol.routes[vid_to]

        if len(r_to.stops) <= 1:
            continue

        # choose insertion position but not depot position 0
    ...
    for _ in range(max_neighbors // 2):
        ...
        # ONLY swap inside same depot
        if depot1 != depot2:
            continue

        # ONLY swap same cluster
        if cust_cluster[cid1] != cust_cluster[cid2]:
            continue
...
def visualize_solution_map(
    ...
    """
    Vẽ bản đồ:
    - Tất cả customers (clustered) dưới dạng điểm.
    - Tất cả depots dưới dạng tam giác.
    - Các route của nghiệm (nối theo thứ tự stops).
    ...
    # màu theo depot cluster
    depot_ids = sorted({d for d in inst.customer_cluster.values()})
    depot_to_color_idx = {d: i for i, d in enumerate(depot_ids)}

    for cid in inst.customers:
        lat, lon = inst.coords.get(cid, (None, None))
        if lat is None or lon is None:
            continue
        cl_depot = inst.customer_cluster.get(cid)
        color_idx = depot_to_color_idx.get(cl_depot, 0)
        cust_lats.append(lat)
        cust_lons.append(lon)
        cust_colors.append(color_idx)
    ...
    # vẽ route: nối các stops theo thứ tự
    ...
    plt.title(f"HCMC – Routes & Clustered Customers ({run_name})")
